...
public class User implements Serializable {
	...
	public void addClassTutor(String department, String number,
			String description) {
		classesTutor.add(new SchoolClass(department, number, description));
	}

	public void addClassStudent(String department, String number,
			String description) {
	...
	public String[] getClassesStudentFormat() {
		...
		if (classesStudent != null) {
			classes = new String[classesStudent.size()];
			int count = 0;
			for (SchoolClass s : classesStudent) {
				classes[count] = s.getSubject() + " " + s.getNumber() + ": "
						+ s.getDescription();
				count++;
			}
		} else {
			classes = new String[] { "No courses added" };
		}
	...
	public String[] getClassesTutorFormat() {
		...
		if (classesTutor != null) {
			classes = new String[classesTutor.size()];
			int count = 0;
